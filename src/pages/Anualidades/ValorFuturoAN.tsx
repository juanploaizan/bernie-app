...
const ValorFuturoAN = () => {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (razonPago > 0 && periodo > 0 && tasa > 0) {
      ...
      let resultado = 0;
      if (esVencida) {
        resultado = (razonPago * (Math.pow(1 + tasa, periodo) - 1)) / tasa;
      } else {
        resultado =
          (razonPago * (Math.pow(1 + tasa, periodo + 1) - tasa - 1)) / tasa;
      }

      setNotificationMessage(
        resultado <= 0
          ? 'El resultado es menor o igual a cero. Asegúrate de diligenciar correctamente los campos'
          : 'El valor futuro, de la anualidad, es de ' +
              '$' +
