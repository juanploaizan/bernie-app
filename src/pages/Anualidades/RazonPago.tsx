...
const RazonPago = () => {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (valorMonto > 0 && periodo > 0 && tasa > 0) {
      ...
      if (esValorFuturo) {
        if (esVencida) {
          resultado = (valorMonto * tasa) / (Math.pow(1 + tasa, periodo) - 1);
        } else {
          resultado =
            (valorMonto * tasa) / (Math.pow(1 + tasa, periodo + 1) - tasa - 1);
        }
      } else {
        if (esVencida) {
          resultado = (valorMonto * tasa) / (1 - Math.pow(1 + tasa, -periodo));
        } else {
          resultado =
            (valorMonto * tasa) / (1 + tasa - Math.pow(1 + tasa, -periodo + 1));
        }
      }
      ...
      setNotificationMessage(
        resultado <= 0
          ...
          : 'La razón de pago de la anualidad, es de ' +
              '$' +
