...
const PeriodoAN = () => {
  ...
  const [valorMonto, setValorMonto] = useState(0);
  const [razonPago, setRazonPago] = useState(0);
  const [tasa, setTasa] = useState(0);
  const [esValorFuturo, setEsValorFuturo] = useState(true);
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (valorMonto > 0 && razonPago > 0 && tasa > 0) {
      ...
      if (esValorFuturo) {
        resultado =
          Math.log((valorMonto / razonPago) * tasa + 1) / Math.log(1 + tasa);
      } else {
        resultado =
          -Math.log(1 - (valorMonto / razonPago) * tasa) / Math.log(1 + tasa);
      }

      if (isNaN(resultado)) {
        setNotificationMessage(
          'El resultado del periodo no es posible de calcular.'
        );
        ...
        setNotificationMessage(
          resultado <= 0
            ...
            : 'El periodo de vida de la anualidad, es de ' +
                resultado.toFixed(0)
        );
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6.5">
            <div className="mb-4.5">
              ...
              <div className="relative z-20 bg-white dark:bg-form-input">
                <select
                  ...
                  onChange={(e) => setEsValorFuturo(e.target.value === '1')}
                >
                  <option value="1">Valor futuro</option>
                  <option value="0">Valor presente</option>
                </select>
              ...
              <input
                type="number"
                onChange={(e) => setValorMonto(Number(e.target.value))}
                placeholder="Ingresa el monto"
                className="mt-3 w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
              />
            </div>
            ...
            <div className="mb-5">
              <label className="mb-2.5 block text-black dark:text-white">
                Razón de pago (vencida)
              </label>
              <input
                type="number"
                onChange={(e) => setRazonPago(Number(e.target.value))}
                placeholder="Ingresa la razón de pago. Esta debe ser vencida."
                className="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
              />
            </div>
