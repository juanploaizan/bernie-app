...
const ValorPresenteIC = () => {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (valorFuturo > 0 && periodo > 0 && tasa > 0) {
      setWarningNotification(false);

      let resultado = valorFuturo / Math.pow(1 + tasa, periodo);

      setNotificationMessage(
        resultado <= 0
          ? 'El resultado es menor o igual a cero. Asegúrate de diligenciar correctamente los campos'
          : 'El valor presente, utilizando interés compuesto, es de ' +
              '$' +
...
export default ValorPresenteIC;
