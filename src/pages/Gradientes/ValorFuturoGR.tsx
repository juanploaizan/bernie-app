...
const ValorFuturoGR = () => {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    if (razonPago > 0 && periodo > 0 && tasa > 0 && constante != 0) {
      setWarningNotification(false);

      let resultado = 0;
      /*
      resultado =
        razonPago * ((1 - Math.pow(1 + tasa, -periodo)) / tasa) +
        (constante / tasa) *
          ((1 - Math.pow(1 + tasa, -periodo)) / tasa -
            periodo / Math.pow(1 + tasa, periodo)); */
      resultado =
        razonPago * ((Math.pow(1 + tasa, periodo) - 1) / tasa) +
        (constante / tasa) *
          ((Math.pow(1 + tasa, periodo) - 1) / tasa - periodo);

      setNotificationMessage(
        resultado <= 0
          ? 'El resultado es menor o igual a cero. Asegúrate de diligenciar correctamente los campos'
          : 'El valor futuro, del gradiente aritmético, es de ' +
              '$' +
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Gradiente aritmético" />
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6.5">
            ...
            <div className="mb-5">
              ...
              <input
                type="number"
                onChange={(e) => setConstante(Number(e.target.value))}
                placeholder="Ingresa el valor positivo o negativo de la constante"
                className="w-full rounded border-[1.5px] border-stroke bg-transparent px-5 py-3 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
              />
            </div>
